...
__attribute__((flatten, hot, optimize("O3,unroll-loops,fast-math"))) int NormalRgaSetAlphaEnInfo(struct rga_req *msg,
...
int NormalRgaSetColorFillMode(
	...
	unsigned char pat_y_off, unsigned char aa_en)
{
	register struct rga_req *m = msg;
	const float scale = 256.0f;

	m->render_mode = color_fill_mode;

	// локальные временные переменные в регистрах
	const int gx_a = (int)(gr_color->gr_x_a * scale) & 0xFFFF;
	const int gx_b = (int)(gr_color->gr_x_b * scale) & 0xFFFF;
	const int gx_g = (int)(gr_color->gr_x_g * scale) & 0xFFFF;
	const int gx_r = (int)(gr_color->gr_x_r * scale) & 0xFFFF;

	const int gy_a = (int)(gr_color->gr_y_a * scale) & 0xFFFF;
	const int gy_b = (int)(gr_color->gr_y_b * scale) & 0xFFFF;
	const int gy_g = (int)(gr_color->gr_y_g * scale) & 0xFFFF;
	const int gy_r = (int)(gr_color->gr_y_r * scale) & 0xFFFF;

	m->gr_color.gr_x_a = gx_a;
	m->gr_color.gr_x_b = gx_b;
	m->gr_color.gr_x_g = gx_g;
	m->gr_color.gr_x_r = gx_r;
	m->gr_color.gr_y_a = gy_a;
	m->gr_color.gr_y_b = gy_b;
	m->gr_color.gr_y_g = gy_g;
	m->gr_color.gr_y_r = gy_r;

	m->color_fill_mode = cf_mode;
	m->pat.act_w = pat_width;
	m->pat.act_h = pat_height;
	m->pat.x_offset = pat_x_off;
	m->pat.y_offset = pat_y_off;
	m->fg_color = color;

	m->alpha_rop_flag |= ((gr_satur_mode & 1u) << 6);

	if (aa_en)
	{
		m->alpha_rop_flag |= 0x1u;
		m->alpha_rop_mode = 1;
	}
...
int NormalRgaUpdatePaletteTableMode(
	register struct rga_req *msg, unsigned long LUT_addr, unsigned int palette_mode)
#else
int NormalRgaUpdatePaletteTableMode(
	register struct rga_req *msg, unsigned int LUT_addr, unsigned int palette_mode)
#endif
{
...
{
	unsigned int mask =
		((src_mmu_en | dst_mmu_en) != 0) * (1u << 31) |
		(src_mmu_en != 0) * (1u << 8) |
		(dst_mmu_en != 0) * (1u << 10);

	msg->mmu_info.mmu_flag |= mask;
	return 1;
}
