...
{ // после подготовки ctx->shot->frame
    ...
    if (!ctx->codec_ctx->hw_device_ctx)
    {
        ...
        {
            RKX_E_TAG(tag, "Failed to create hw_device_ctx");
...
// int init_text_filter(const char *text, AVBufferRef *hw_device_ctx, s_text_f *text_t)
// {
// }
// int init_text_filter(const char *text, AVBufferRef *hw_device_ctx, s_text_f *text_t)
// {
//     (void)hw_device_ctx; // пока не используем — CPU путь

//     if (!text_t || !text_t->frame)
//     {
//         fprintf(stderr, "init_text_filter: invalid text_t or frame\n");
//         return -1;
//     }

//     AVFrame *frame = text_t->frame;
//     if (!frame->data[0])
//     {
//         fprintf(stderr, "init_text_filter: empty frame buffer\n");
//         return -1;
//     }
...
//         img.virt_addr = frame->data[0];
//         break;

//     case AV_PIX_FMT_RGBA:
//         img.format = IMAGE_FORMAT_RGBA8888;
//         img.virt_addr = frame->data[0];
//         break;

//     case AV_PIX_FMT_NV12:
//         img.format = IMAGE_FORMAT_YUV420SP_NV12;
//         img.virt_addr = frame->data[0]; // Y + UV
//         break;

//     case AV_PIX_FMT_NV21:
//         img.format = IMAGE_FORMAT_YUV420SP_NV21;
//         img.virt_addr = frame->data[0];
//         break;

//     default:
//         fprintf(stderr, "init_text_filter: unsupported fmt %d\n", frame->format);
//         return -1;
//     }
...
//     draw_text(&img,
//               text ? text : text_t->text,
//               text_t->x,
//               text_t->y,
//               text_t->color,
//               text_t->fontsize ? text_t->fontsize : 24);
