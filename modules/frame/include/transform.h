...
// int allocate_drm_buffer(int drm_fd, int width, int height, int format, int *dmabuf_fd);

namespace rockf
{
    // int8_t resizes(frame_t *input, frame_t *output, int new_width, int new_height, AVPixelFormat format, bool mode);
    ...
    /**
     * @brief Конвертировать цветовые форматы.
     *
     * @param frame_t *input Входное изображение
     * @param frame_t *output Выходное изображение
     * @param AVPixelFormat format выходной цветовой формат
     *
     */
