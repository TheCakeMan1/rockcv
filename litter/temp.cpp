...
//     convert_buffer_to_avframe(output);
//     if (output->buffer) {
//         av_free(output->buffer);  // Освобождаем буфер
//     }
//     if (input->buffer) {
//         av_free(input->buffer);  // Освобождаем буфер
//     }

//     // src.virAddr = nullptr;  // Обнуление указателя после освобождения
//     // dst.virAddr = nullptr;
//     // if (src.virAddr != nullptr) {
//     //     free(src.virAddr);  // Освобождение памяти, если она была выделена
//     //     src.virAddr = nullptr;  // Обнуление указателя после освобождения
//     // }

//     // if (dst.virAddr != nullptr) {
//     //     free(dst.virAddr);  // Освобождение памяти, если она была выделена
//     //     dst.virAddr = nullptr;  // Обнуление указателя после освобождения
//     // }

//     return 0;
// }

// int dma_buf_alloc(const char *heap, size_t size, int *fd, void **buf) {
//     int heap_fd = open(heap, O_RDWR);
//     if (heap_fd < 0) {
//         std::cerr << "Не удалось открыть DMA heap: " << strerror(errno) << std::endl;
//         return -1;
//     }
...
//         std::cerr << "Не удалось выделить DMA-буфер: " << strerror(errno) << std::endl;
//         close(heap_fd);
//         return -1;
//     }

//     *fd = alloc_data.fd;
//     close(heap_fd);  // Закрываем heap_fd, он больше не нужен

//     *buf = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, *fd, 0);
//     if (*buf == MAP_FAILED) {
//         std::cerr << "Не удалось маппить DMA-буфер: " << strerror(errno) << std::endl;
//         close(*fd);
//         return -1;
//     }
...
//     rga_info_t src = {}, dst = {};
//     int dma_fd_input = -1, dma_fd_output = -1;
//     void *input_buf = nullptr, *output_buf = nullptr;

//     // Чистим старый output
//     if (output->avframe) {
...
//     int ret = av_image_alloc(output->avframe->data, output->avframe->linesize, new_width, new_height, format, 16);
//     if (ret < 0) {
//         av_frame_free(&output->avframe);
//         std::cerr << "Ошибка выделения памяти для выходного кадра!" << std::endl;
//         return 1;
...
//     // Чистим буферы
//     int input_size = av_image_get_buffer_size((AVPixelFormat)input->avframe->format, input->avframe->width, input->avframe->height, 1);
//     int output_size = av_image_get_buffer_size(format, new_width, new_height, 1);

//     if (dma_buf_alloc("/dev/dma_heap/system", input_size, &dma_fd_input, &input_buf) < 0) {
//         av_frame_free(&output->avframe);
//         return 1;
//     }

//     if (dma_buf_alloc("/dev/dma_heap/system", output_size, &dma_fd_output, &output_buf) < 0) {
...
//     // Копируем входные данные в DMA-буфер
//     av_image_copy_to_buffer((uint8_t*)input_buf, input_size, input->avframe->data, input->avframe->linesize,
//                             (AVPixelFormat)input->avframe->format, input->avframe->width, input->avframe->height, 1);

//     // Настройка RGA
//     src.fd = dma_fd_input;
//     src.virAddr = input_buf;
//     src.mmuFlag = 1;
//     src.format = AV_TO_RK_FORMAT(input->avframe->format);
//     rga_set_rect(&src.rect, 0, 0, input->avframe->width, input->avframe->height,
//                  ALIGN_UP(input->avframe->width, 16), ALIGN_UP(input->avframe->height, 16), src.format);
...
//     dst.virAddr = output_buf;
//     dst.mmuFlag = 1;
//     dst.format = AV_TO_RK_FORMAT(format);
//     rga_set_rect(&dst.rect, 0, 0, new_width, new_height, new_width, new_height, dst.format);

//     // Выполняем масштабирование через RGA
//     int rga_status = c_RkRgaBlit(&src, &dst, NULL);
//     if (rga_status != 0) {
//         std::cerr << "Ошибка: RGA масштабирование не удалось! Код ошибки: " << rga_status << std::endl;
//         goto cleanup;
...
//     // Копируем данные из DMA-буфера в выходной `avframe`
//     av_image_copy(output->avframe->data, output->avframe->linesize, (const uint8_t**)&output_buf,
//                   output->avframe->linesize, format, new_width, new_height);
